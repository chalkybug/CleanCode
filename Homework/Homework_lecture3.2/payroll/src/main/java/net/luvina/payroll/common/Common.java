...
public class Common {
     ...
     *
     * @param date số năm và tháng kinh nghiệm
    ...
    /**
     * Chuyển đổi date thành text hiển thị số năm kinh nghiệm.
     *
     * @param date số năm kinh nghiệm
     ...
     *
     * @param role chức vụ
     * @return hệ số lương nếu tồn tại. Trả về 0 nếu không tồn tại chức vụ
     */
    public static double getRateSalary(String role) {
        RankSalaryEnum[] allRank = RankSalaryEnum.values();
        for (RankSalaryEnum rankSalary : allRank) {
            //Comparing
            if (rankSalary.toString().equalsIgnoreCase(role)) {
                return rankSalary.getValue();
            }
        }
        return 0;
    }
